...
- **ESM `import` hoisting defeats module-level env var reads** — in ESM (including Bun), static `import` statements are hoisted and run *before* any module-level code, even if you write `process.env.X = "..."` above the import in source. So a module-level constant like `const DAEMON_BASE = \`http://localhost:${process.env.QREC_PORT}\`` is frozen before the calling module can set the env var. Pattern: use a function (`const getDaemonBase = () => \`http://localhost:${process.env.QREC_PORT ?? "25927"}\``) so the env var is read at call time, not module load time. `mcp.ts` was fixed this way (DAEMON_BASE → getDaemonBase()).
- **Test pattern for env-sensitive module constants** — when a test needs to set an env var before a module's constants are frozen, use `await import()` inside `beforeAll()` after setting the var. A static import is always hoisted; the env var assignment below it has no effect on the module's initialization.
- **`Bun.spawn` does NOT inherit `process.env` mutations made after process startup** — unlike Node's `child_process.spawn`, Bun snapshots the environment at process start; mutations (e.g. `process.env.QREC_PORT = N` from `--port`) are invisible to spawned children unless you pass `env: process.env` explicitly. Always include `env: process.env` in `Bun.spawn` calls in `daemon.ts`.
