...
- **Enrich debounce** — `spawnEnrichIfNeeded()` gates on `lastIndexActivityAt`: only spawns if no session has been re-indexed for ≥ `QREC_ENRICH_IDLE_MS` (default 300s). `lastIndexActivityAt` resets whenever the indexer progress callback fires (`current` non-empty). Without this, enrich re-runs every 60s cron cycle while a session is actively being written.
