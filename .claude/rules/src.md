...
- **`GET /sessions` response format** — returns `{ sessions, total, offset, limit }` (full metadata, sorted date DESC). Supports `?offset=N` for pagination (page size 100). `tags`/`entities` are parsed arrays (null when unenriched). `/sessions/:id` also includes `summary`, `tags`, `entities`.
